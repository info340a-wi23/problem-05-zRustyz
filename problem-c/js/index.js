...
let opponent = huskyGames2016.map(function(object) {
  return extractOpponent(object);
});
...
function huskiesLost(game) {
  ...
  let oScore = 0;
  if (game.home === "UW") {
    wScore = game.home_score;
    oScore = game.opponent_score;
  } else {
    oScore = game.home_score;
    wScore = game.opponent_score;
  }
  if (wScore < oScore) {
    return true;
  } else {
...
let lostGames = huskyGames2016.filter(function(object) {
  if (huskiesLost(object)) {
    return object;
  }
});
...
let lostOpponenets = lostGames.map(function(object) {
  return extractOpponent(object);
});
console.log(lostOpponenets);
...
lostGames.forEach(function(object) {
  console.log(
    object.opponent + " at " + object.home + ", " + object.opponent_score
    + " to " + object.home_score);
});
...
let fumbleGames = huskyGames2016.filter(function(object) {
  if (object.fumbles > 0) {
    return object;
  }
});
...
function mostYardsPassing(first, second) {
  if (first.passing_yards === 0 || first.passing_yards < second.passing_yards) {
    return second;
  } else {
    return first;
  }
...
let mostPassingGame = huskyGames2016.reduce(function(prev, curr) {
  return mostYardsPassing(prev, curr);
});
...
function makeCombinedFilter(firstFunc, secondFunc) {
  return function combinedFilter(game) {
    return firstFunc(game) && secondFunc(game);
  };
}
...
function hasFumble(game) {
  if (game.fumbles > 0) {
    return game;
  }
}

const fumbledAndLostFilter = makeCombinedFilter(huskiesLost, hasFumble);
...
let lostAndFumble = huskyGames2016.filter(function(game) {
  return fumbledAndLostFilter(game);
});
